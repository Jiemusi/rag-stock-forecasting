...
# ============================================================
# 3. BACKTEST RESULTS (Multi-Symbol Panel Backtest + Offline)
# ============================================================

if page == "📈 Backtest Results":
    st.title("Cross-Sectional Backtest (Panel + Offline Summary)")

    st.markdown(
        """
            🔁 **Interactive panel backtest** using a pre-computed panel file
           (e.g., generated by `src/eval_panel.py` with `--output_panel`).  
           By default we focus on the 10-name universe:
           `NVDA, AAPL, AMD, MSFT, GOOGL, TSM, AVGO, ASML, QCOM, TXN`.

        """
    )

    # -----------------------------
    # 3.1 Interactive panel backtest
    # -----------------------------
    st.write("### 🔁 Interactive Panel Backtest")

    panel_path_bt = st.text_input("Panel file path", "data/panel_eval_rank.pkl")
    start_date_bt = st.date_input("Start Date", datetime.date(2024,12, 1), key="bt_start_date")
    end_date_bt = st.date_input("End Date", datetime.date(2024,12, 10), key="bt_end_date")
    rank_norm_bt = st.checkbox("Use rank-normalized predictions?", True, key="bt_rank_norm")

    if st.button("Run Backtest", key="bt_run_button"):
        ...
        # Filter to default 10-name universe defined in `symbols`
        if symbols:
            panel_bt = panel_bt[panel_bt["symbol"].isin(symbols)]

        if panel_bt.empty:
            st.warning("No panel rows found for the given date range and default symbol universe.")
        else:
            st.success(f"Loaded {len(panel_bt)} panel rows for backtest over the default 10 names.")

            bt_panel = run_backtest(panel_bt, rank_normalize=rank_norm_bt)

            st.write("#### Backtest Dashboard")
    ...
    # -----------------------------
    # 3.2 Offline saved charts
    # -----------------------------
        # st.write("### 📊 Saved Offline Backtest Charts")

        saved_figs = {
            "IC Timeseries ": "results/ic_timeseries.png",
            "IC Histogram ": "results/ic_histogram.png",
            "Long-Short Equity Curve ": "results/longshort_equity_curve.png",
        }

        for title, path in saved_figs.items():
            if os.path.exists(path):
                st.write(f"#### {title}")
                st.image(path, use_column_width=True)
            else:
                st.write(f"*(Missing file: {path})*")
...
#     end_date = st.date_input("End Date", datetime.date(2024,12, 10))
